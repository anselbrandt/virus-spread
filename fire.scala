...
object fire extends App {
  object Forest {
    ...
    val BURNING = '#'
    val EMPTY = '.'
    def apply(x: Int = 30, y: Int = 15) = new Forest(
      Array.tabulate(y, x)((y, x) =>
        if (Random.nextDouble() < 0.5) TREE else EMPTY
      )
    )
  }
  ...
  class Forest(matrix: Array[Array[Char]]) {
    ...
    def evolve(): Forest = new Forest(Array.tabulate(rows, cols) { (y, x) =>
      matrix(y)(x) match {
        ...
        case BURNING => EMPTY
        case TREE =>
          if (neighbours(x, y).exists(_ == BURNING)) BURNING
    ...
    def neighbours(x: Int, y: Int) =
      matrix.slice(y - 1, y + 2).map(_.slice(x - 1, x + 2)).flatten
